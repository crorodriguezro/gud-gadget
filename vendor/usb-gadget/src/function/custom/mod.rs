...
impl EndpointReceiver {
    ...
    ///
    /// The file is opened while FunctionFS is initialized. Reusing it avoids a
    /// second `open()` of an enabled endpoint, which can block indefinitely on
    /// some UDC drivers.
    pub fn file(&mut self) -> Result<Arc<File>> {
        self.0.get()?.file()
    }
